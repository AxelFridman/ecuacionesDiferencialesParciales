...
# ╔═╡ a3f3bd74-aa28-4712-bdd8-c2fad5312b6a
using Distributions

# ╔═╡ 1bc830d9-2ef7-4185-8260-fa7595bb2790
using PlutoUI

# ╔═╡ 5aa58b7c-93e8-4485-872d-9b17ad682c35
#using PlotlyJS
...
## Trabajo Práctico $n^\circ 3$ - Tema 1: Convección-difusión.
"""
...
# ╔═╡ 525ec5c7-14e3-401d-92ca-af25f5bcc603
md"""
...
2. El borde $\theta=0$ y el borde $\theta=2\pi$ son el mismo. Hay dos maneras de considerar esto: 
 * tomar $\theta_1=0$, $\theta_J=2\pi$ y mantener en el sistema dos valores que en la práctica están repetidos (se espera que $u_{i,1}=u_{i,J}$ para todo $i$). Esto hay que considerarlo en las ecuaciones, en las que $u_{i,0}$ debe reemplazarse por $u_{i,J-1}$ y  $u_{i,J+1}$ debe reemplazarse por $u_{i,2}$. Este planteo es redundante, pero puede resultar cómodo al momento de graficar la solución. 
 * tomar $\theta_1=0$, $\theta_J=2\pi-\Delta\theta$. En este caso no se guardan datos repetidos. Debe identificarse $u_{i,0}$ con $u_{i,J}$ y $u_{i,J+1}$ con $u_{i,1}$. 
...
$\frac{u_{2,j}-u_{0,j}}{\Delta r} = 0,$ lo que implica $u_{0,j}=u_{2,j}$. La manera de imponer esta condición es simplemente poner $u_{2,j}$ cada vez que en las ecuaciones se espere $u_{0,j}$."""





# ╔═╡ 3779d56b-429e-441f-bc70-45d066f2402a
md"""
###### Ejercicio 1: 

Escribir la discretización de las ecuaciones e implementar la función que corresponde al sistema para generar un ODEProblem. El vector de parámetros `p` debe contener la información necesaria para calcular las ecuaciones: el vector `r`, el paso `Δr` (puede inferirse de `r`), el paso `Δθ`, y los parámetros `α` y `β`. """

# ╔═╡ c1a76c83-f1e9-48fe-8880-483a8a3ade05
...
Implementar una función que reciba una solución al problema y un vector de tiempos y genera una película a partir de heatmaps. 
"""

# ╔═╡ 9c50fcf7-0711-4045-b579-3ca1013caa9c
...
Resolver para $\alpha = 0.05$ y $\beta=3$. Considerar un dato inicial que valga 1 en cierta zona del dominio y 0 en el resto. 

Probar también otros valores de $\alpha$ y $\beta$. Comparar la dinámica. 
"""

# ╔═╡ ef633703-a676-4498-be94-6295580e02f6
...
Modificar las ecuaciones de manera de aceptar una matriz $\beta$ (del mismo tamaño de `u`). Esto permitirá considerar campos de velocidades que varíen a lo largo de todo el dominio. 

Probar un campo de velocidades igual a `r` y un campo de velocidades de la forma `r(1-r)`. Por último, probar alguna alternativa con variación angular. 
"""

# ╔═╡ 194c56cf-e5df-4743-9707-308cd8440b72


# ╔═╡ 6815e97b-28f4-4cc5-9196-325df8dae649
...
## Trabajo Práctico $n^\circ 3$ - Tema 2: Patrones de Turing con método ADI.
"""

# ╔═╡ 17dc1e39-4174-4cf7-b035-6aebbe9de376
md"""
En 1952 Turing publicó un artículo titulado _The chemical basis of morphogenesis_ en el que muestra que ciertos sistemas químicos de reacción-difusión pueden dar lugar a la formación de patrones en la distribución de concentraciones de las distintas substancias  intervinientes. 

El objetivo de este TP es simular numéricamente un ejemplo de este tipo. 

Las ecuaciones del modelo son las siguientes: 

$u_t = \alpha_u\Delta u +  F(u,v)$

$v_t = \alpha_v\Delta v +  G(u,v)$

Puede observarse que cada substancia sigue un proceso de difusión, al que se le agregan términos de reacción: $F(u,v)$ y $G(u,v)$, que son funciones no lineales. 

La idea de este TP es simular un proceso de este tipo utilizando el método ADI. """
...
La idea del método ADI es resolver matricialmente problemas en 2 o 3 dimensiones, pero sin _desenrollar_ la solución como un largo vector. 

Concretamente, si denotamos:

$\delta_x^2(u_{i,j}) = \frac{1}{h^2}\big(u_{i-1,j}-2u_{i,j}+u_{i+1,j}\big),$

la aproximación usual de $u_xx$ y de manera análoga $\delta_y^2(u_{i,j})$ la aproximación usual de $u_yy$, el método ADI propone resolver el siguiente sistema:

$\begin{array}{rcl} 
\frac{u^*-u^n}{\Delta t/2} &=& \delta_x^2(u^*)+\delta_y^2(u^n) + F(u^n,v^n) \\
\frac{v^*-v^n}{\Delta t/2} &=& \delta_x^2(v^*)+\delta_y^2(v^n) + G(u^n,v^n) \\
\frac{u^{n+1}-u^*}{\Delta t/2} &=& \delta_x^2(u^*)+\delta_y^2(u^{n+1}) + F(u^*,v^*)\\
\frac{v^{n+1}-v^*}{\Delta t/2} &=& \delta_x^2(v^*)+\delta_y^2(v^{n+1}) + G(u^*,v^*)
\end{array}$

La idea es que $u^*$ y $v^*$ operen como pasos intermedios. En cada (medio) paso aplicamos una derivada de manera explícita y la otra de manera implícita. 

Si consideramos $u\in\mathbb{R^{n,m}}$, una matriz $D_x$ que corresponda a las derivadas en $x$ opera sobre las filas de $u$ y por lo tanto debe aplicarse como $D_x u$. En cambio, una matriz $D_y$ que corresponda a las derivadas respecto de $y$ opera sobre las columnas de $u$ y por lo tanto debe aplicarse como $uD_y^t$.
...
Despejar el sistema anterior de manera de expresarlo como:

$\begin{array}{rcl}
A_I u^* &=& u^n A_E^t + \frac{\Delta_t}{2} F(u^n,v^n)\\
u^{n+1} A_I^t &=& A_E u^* +\frac{\Delta t}{2} F(u^*,v^*),\end{array}$

y las ecuaciones análogas para $v$, siendo $A_E$ y $A_I$ las matrices correspondientes a la parte explíctica e implícita respectivamente. 

Para la construcción de las matrices $A_E$ y $A_I$ consideramos condiciones de contorno de tipo Neumann homogéneas, modeladas mediante las ecuaciones: 
...
md"""###### Ejercicio 5:
Probar el programa tomando $u^0$ y $v^0$ matrices aleatorias con números entre $0$ y $1$ y las funciones: 

$F(u,v) = u-u^3-v+k$
$G(u,v) = u-v$

(Sistema de FitzHugh-Nagumo). Tomar, además $\alpha_u=2.8e-4$, $\alpha_v=5e-3$ y $k=-0.05$. Para tener buena definición, conviene resolver con $h=0.01$ y $\Delta t = 0.01$. 

Resolver también para $k=-0.005$ y comparar la estructura de los patrones. ¿Se nota alguna diferencia? 
"""
...
Probar también con el sistema: 

$F(u,v) = γ(a - bu + u^2v)$

$G(u,v) = γ(c - u^2v)$

Usar los valores: $\gamma = 0.2$, $a=0.09$, $b=0.055$, $c=0.075$.  Jugar un poco con los valores de algún parámetro. 
"""

# ╔═╡ 3ff9a5a1-bc9e-4366-b8e1-c5483452ae30
@bind γ PlutoUI.Slider(0.2:1)

# ╔═╡ 578d82ba-bba4-4d5b-b79c-ce43318f389f
@bind a PlutoUI.Slider(0.09:1)

# ╔═╡ 063d1c7e-d6cf-4987-8e8b-bb310dbd1905
@bind b PlutoUI.Slider(0.055:1)

# ╔═╡ bc9cbd6e-4342-4cb9-987d-1bdccd402a3d
@bind c PlutoUI.Slider(0.075:1)

# ╔═╡ e9b462fa-6609-4bb2-9caf-d5c5f8fec45c
@bind alphaU6 PlutoUI.Slider(1*10^-4:1*10^-3)

# ╔═╡ 32e2da19-0244-4ff3-9e3e-3cc5665bfb69
@bind alphaV6 PlutoUI.Slider(1*10^-4:1*10^-3)
